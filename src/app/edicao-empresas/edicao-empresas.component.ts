...
export class EdicaoEmpresasComponent implements OnInit {
  ...
  constructor(
    ...
    private activatedRouter: ActivatedRoute //inicialização
  ) { }
  ...
  //função utilizada para exibir as mensagens de erro
  //de validação na página
  get form(): any {
  ...
  ngOnInit(): void {
    ...
    var idEmpresa = this.activatedRouter.snapshot.paramMap.get('id');

    //obtendo os dados da empresa na API
    this.empresasService.getById(Number(idEmpresa)).subscribe(
      (data: any) => {
  ...
  onSubmit(): void {
    ...
    this.empresasService.put(this.formEdicao.value).subscribe(
      (data: any) => {
